...
Every long-lived artifact has one versioned ArtifactSpecification in transactional state:
...
## Source: `lpos/LPOS-031-deterministic-control-plane.md`
...
Safety-critical authority lives outside model prompts. The control plane owns state
transitions, materiality, identity verification, approval validity, exact-action hashing,
idempotency, context boundaries, persistence, scheduling claims, audit events, and external
action execution. Models may propose; they cannot bypass these controls.

Every normative rule maps to runtime code, a schema, build validation, or an executable
evaluation. A rule without an enforcement point is explicitly advisory.

---

## Source: `lpos/LPOS-032-transactional-state.md`

---
id: LPOS-032
title: Transactional State and Audit
version: 1.0.0
status: Accepted
classification: Runtime Standard
owner: Listening Post
...
The authoritative runtime state is transactional. Events are append-only. Concurrency uses

...
class GateResult:
    action: str          # "accept_new_best" | "reject"
    accepted: bool
    current_score: float
...
def evaluate_gate(*, current_score: float, candidate_score: float,
                  min_gain: float = 0.0) -> GateResult:
    """Strictly-better gate. A tie is a rejection: an edit must earn its place.

    min_gain lets the caller demand a margin (e.g. 0.01) so noise-sized wins do
    not accumulate as instruction bloat.
    """
    if candidate_score > current_score + min_gain:
        return GateResult(
            "accept_new_best", True, current_score, candidate_score,
            f"candidate {candidate_score:.4f} beat current {current_score:.4f} "
            f"by more than min_gain {min_gain:.4f}",
        )
    return GateResult(
        "reject", False, current_score, candidate_score,
        f"candidate {candidate_score:.4f} did not beat current "
        f"{current_score:.4f} + min_gain {min_gain:.4f}",
    )

...
def _scan_kanban(root: Path) -> list[dict[str, Any]]:
    ...
    for file in files:
        ...
        for index, item in enumerate(_kanban_items(data)):
            ...
            if isinstance(raw_path, str) and raw_path.strip():
                card_path = Path(raw_path.strip()).expanduser()
            ...
            projects.append(
                {
                    ...
                    "name": name,
                    "path": str(card_path),
                    "friendly_path": _friendly_path(card_path),
                    "description": description.strip(),
                    "agent": agent,
                    "type": project_type,
    ...
    for scan_root in project_roots(base):
        ...
        for entry in entries:
            if entry.name.startswith(".") or not entry.is_dir():
                continue
            project = _scan_folder_project(entry, scan_root.name)
    ...
    for project in projects:
        ...
        for dirpath, dirnames, filenames in os.walk(project_path):
            ...
            dirnames[:] = [
                d
                for d in dirnames
                if not d.startswith(".") and d not in _SKIP_DIR_NAMES and depth < _SEARCH_MAX_DEPTH
            ]

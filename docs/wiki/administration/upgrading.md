...
- The installer creates one LPOS code and environment directory per release. Mutable state remains in the configured external state root.
...
3. **Back up your database** from the current install (see [Backups](/administration/backups.html)):
    ...
    ```bash
    lpos export --db ~/.local/state/lpos/lpos.db --output pre-upgrade-events.jsonl
    ```

    and copy `~/.local/state/lpos/lpos.db` itself.
...
5. **Point the new install at your state** and validate migrations against a *copy* first if you want the extra caution, then run:
    ...
    ```bash
    lpos init --db ~/.local/state/lpos/lpos.db
...
6. **Run the evaluations**: `lpos evals` should pass all 70 deterministic evaluations for v4.6.0.

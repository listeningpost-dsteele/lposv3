...
`doctor` confirms the whole system is healthy in one JSON object: the specification kernel is loaded, 32 specialists, 21 standing operations, 53 benchmarks, 17 valid schemas, and your database's migrations and integrity. `list-specialists` shows every specialist with its guild, capabilities, and model class. `list-workflows` shows every Standing Operation with its default schedule. This is the entire inventory — there is nothing hidden.

...
- The Constitution contains Article VIII and Laws 16–18 establishing that no guild or newly created organization is trusted by designation and that its material output must pass the normal LPOS adversarial process.
- `GUILD-039`, `SPECIALIST-033`, and `SO-026` are present, registered, documented, and covered by the required no-self-trust regression suite.
- Sentinel raw assessments, rules, reports, and proposed remediation remain untrusted until exact-hash, fresh-context independent review and deterministic structural verification pass.
...
- CODEOWNERS and branch protections that prevent `GUILD-039`, its implementation owners, or any other new guild from approving its own constitutional, policy, scanner-rule, report, or remediation changes.
- Protected CI checks for the no-self-trust, exact-hash review-binding, fresh-context isolation, deterministic re-scan, passive-boundary, and completion-gate tests in Section 18.
...
- the Article VIII no-guild-trust rule and the requirement that Sentinel and every future new guild use the ordinary LPOS adversarial process
- Sentinel operating instructions, Principal report workflow, remediation workflow, passive-testing boundary, and assurance-failure behavior
...
- the constitutional amendment, Sentinel charter, Sentinel test evidence, independently reviewed sample reports, and no-self-trust verification record
...
patch_completion:
  ...
  sentinel_adversarial_assurance:
    constitution_article_viii_verified: false
    laws_16_18_verified: false
    guild_039_verified: false
    specialist_033_verified: false
    so_026_verified: false
    raw_output_untrusted_test: ""
    exact_hash_review_binding_test: ""
    fresh_context_independence_test: ""
    deterministic_rescan_test: ""
    self_approval_refusal_test: ""
    principal_report_and_remediation_test: ""
    critical_high_completion_gate_test: ""
    passive_non_destructive_boundary_test: ""
    assurance_failure_fail_closed_test: ""
    github_paths: []
    google_drive_paths: []
    wiki_pages: []
    next_full_build_manifest_verified: false
  findings:
    LPOS-01: "Open"
    LPOS-02: "Open"
    LPOS-03: "Open"
...
## 18. Constitutional amendment and Sentinel Adversarial Assurance

This section is an additive security architecture requirement. It does **not** close
LPOS-01 through LPOS-14, does not replace the P0/P1 remediation above, and must not be
used by itself as evidence that LPOS is secure, SOC 2 compliant, or ready for a Type 2
opinion.

### 18.1 Constitution: no guild is trusted by designation

Hermes must incorporate the following principles into the canonical Constitution as
Article VIII, **Institutional Skepticism and Independent Adversarial Review**, and into
binding Laws 16–18:

1. No guild, specialist, agent, model, provider, security team, assurance function, or
   newly created organization is trusted merely because of its name, mission, source,
   independence claim, or security designation.
2. Material output from every new organization begins in an explicit `untrusted`
   state. This includes findings, reports, scanner rules, policy changes, completion
   decisions, remediation recommendations, attestations, and claims that work is safe.
3. A producing organization may not approve, certify, publish as fact, enforce,
   remediate, suppress, downgrade, close, or grant authority to its own output.
4. Before such output can authorize or block an action, affect task completion, reach
   the Principal as a factual finding, be externally published, or change policy, the
   exact output hash must pass the **ordinary LPOS adversarial process**:
   - a normal `ReviewEnvelope` bound to the exact producer output and source artifact;
   - a fresh-context review that excludes the producer's creation context and
     self-assessment;
   - a reviewer selected independently of the producing organization;
   - deterministic structural verification and, where applicable, a reproducible
     re-scan of the exact artifact revision; and
   - an immutable review decision recording pass, rejection, or review failure.
5. A reviewed finding never authorizes its own remediation. Remediation must become a
   separately scoped LPOS task and pass the usual interpretation, specification,
   approval, execution, and independent-review controls.
6. The Principal retains final authority. A guild may advise or create evidence, but it
   cannot grant itself institutional trust.

The Constitution and Laws are release-blocking source artifacts. Documentation that
calls Sentinel “trusted,” “authoritative,” “self-validating,” or equivalent fails this
patch.

### 18.2 Create an independent organization: GUILD-039

Add **GUILD-039 — Sentinel Adversarial Assurance** as an organization separate from
Chip's work-producing context. Register:

- `GUILD-039`: Sentinel Adversarial Assurance;
- `SPECIALIST-033`: Adversarial Assurance Engineer; and
- `SO-026`: Continuous Adversarial Assurance.

Sentinel's mandate is to continuously monitor exact revisions of artifacts created or
materially changed through Chip, identify high-signal security weaknesses, and provide
the Principal with independently reviewed remediation guidance. It must not be routed
as the creator and reviewer of the same output.

### 18.3 Continuous monitoring and safe penetration-test boundary

Sentinel must operate through both:

- an event-driven hook after each Chip artifact revision is persisted; and
- a recurring Standing Operation that inventories and evaluates any artifact revision
  missed by the event hook.

The default continuous mode is passive and non-destructive. It may perform static
analysis, policy checks, dependency/manifest inspection, threat modeling, safe parsing,
and deterministic test execution in an approved isolated test fixture. It must not, by
its mere standing mandate:

- exploit a live service;
- scan arbitrary networks or third-party systems;
- use production credentials;
- invoke an unrestricted shell;
- establish persistence;
- exfiltrate data;
- bypass approval, logging, or access controls; or
- mutate production state.

Active penetration testing requires a separate Principal-approved engagement defining
owner, target authorization, scope, excluded assets, test window, isolated environment,
credentials, data handling, stop conditions, communications, rollback, and evidence
retention. The exact scope hash must be bound to an action and grant persisted through
LPOS's ordinary action and approval ledgers and revalidated immediately before use.
Scope approval does not waive Article VIII review of the resulting work.

### 18.4 Required no-self-trust data and review pipeline

Implement the following append-only pipeline:

```text
Chip artifact revision
  -> raw Sentinel SecurityAssessment (trust_state=untrusted)
  -> immutable assessment hash
  -> ordinary fresh-context ReviewEnvelope
  -> independent review result
  -> deterministic exact-revision re-scan and structural verification
  -> trusted SecurityAssessmentReview OR assurance failure
  -> PrincipalSecurityReport with reviewed findings and remediation only
```

Required invariants:

- A raw assessment is never accepted with a trust state other than `untrusted`.
- Review records bind the assessment hash, artifact hash, review-envelope hash,
  reviewer identity, fresh-context identifier, structural checks, and decision.
- The Sentinel producer identity and producing organization are excluded from reviewer
  selection.
- A pass from a model reviewer cannot override a failed deterministic check.
- A deterministic check cannot manufacture trust without the ordinary independent
  review result.
- Rejected, missing, stale, same-producer, self-approved, context-contaminated, or
  structurally inconsistent reviews fail closed.
- On review failure, LPOS may report an **assurance failure**, but must not present the
  raw guild allegations as factual findings.
- Assessments, reviews, reports, and acknowledgements are append-only. Acknowledgement
  does not suppress, remediate, downgrade, or close a finding.

This no-self-trust pipeline is the minimum pattern for every future new guild. Hermes
must reuse it or an equivalently strong canonical primitive rather than adding a
trusted-guild exception.

### 18.5 Principal reports and remediation behavior

A Principal-facing report may contain a security finding only after the pipeline in
Section 18.4 succeeds. Each reviewed finding must contain:

- severity, category, confidence, rule identifier, and stable fingerprint;
- exact task and artifact-revision binding;
- privacy-preserving, redacted evidence with evidence hashes;
- a clear description of impact and uncertainty;
- concrete remediation steps; and
- verification or regression-test steps.

Independently reviewed Critical and High findings block completion of the affected
artifact revision and route the task to the normal correction process. Medium, Low,
and Informational findings are advisory by default unless a separately approved policy
raises their treatment. Sentinel may recommend remediation but may not edit the source,
approve the fix, close the finding, or certify its own rule. The corrected artifact is
a new revision and must receive a new assessment and the ordinary independent review.

### 18.6 Minimum technical controls

The implementation must include:

- immutable records for raw assessments, independent reviews, Principal reports, and
  report acknowledgements;
- deterministic high-signal checks for private-key material, hard-coded credentials,
  unsafe shell execution, download-to-shell behavior, unsafe deserialization,
  SQL interpolation, disabled TLS verification, weak cryptography, permissive file
  modes, and attempts to bypass control/review/logging paths;
- evidence redaction that prevents detected secrets from being copied into reports or
  logs;
- exact artifact-revision lookup and binding;
- an explicit assurance-failure status;
- a Principal security inbox and acknowledgement trail;
- configurable blocking severities with secure defaults of Critical and High;
- CLI/operator inspection, scan, report, status, and acknowledgement commands;
- packaged schemas, migrations, release metadata, documentation, and offline tests; and
- telemetry sufficient to detect missed assessments, rejected reviews, stale policy
  versions, and unacknowledged reports without asserting that absence of findings proves
  security.

### 18.7 Mandatory adversarial regression tests

Hermes must add protected tests that prove at least the following:

1. Every persisted Chip artifact revision is automatically assessed, and the Standing
   Operation catches a deliberately unassessed revision.
2. A raw guild assessment cannot be stored or reconstructed as trusted.
3. A same-producer or Sentinel-selected self-review is rejected.
4. A review that does not attest the exact fresh context is rejected.
5. A review for a different assessment hash or artifact hash is rejected.
6. A model review pass plus a deterministic re-scan mismatch remains untrusted.
7. A deterministic pass without an independent review remains untrusted.
8. A rejected or unavailable review produces only an assurance-failure notice; raw
   findings are not exposed as verified facts.
9. Known malicious fixtures produce redacted evidence and useful remediation without
   executing the fixture, opening a network connection, or invoking a shell.
10. Independently reviewed Critical and High findings block completion of only the
    affected revision and route it through correction.
11. Medium and lower findings remain advisory under the default policy.
12. A corrected revision is re-assessed and may complete only after its own independent
    review passes.
13. Sentinel cannot update/delete assessments, reviews, reports, or acknowledgements;
    it cannot acknowledge, downgrade, suppress, remediate, or close its own finding.
14. Constitutional text, guild charters, registry entries, workflow catalog, wiki,
    package data, schemas, and release manifests are mutually consistent.
15. An active probe is refused without a separately approved authorization and isolated
    engagement scope; the exact action and grant must exist in the ordinary LPOS ledgers
    and be revalidated through the normal approval service.

Tests must include a malicious or compromised guild-output fixture that asserts it is
“trusted,” instructs the reviewer to skip normal controls, or attempts to approve
itself. The ordinary adversarial process must reject the attempted trust escalation.

### 18.8 Hermes publication and next-full-build requirements

Hermes must publish this constitutional amendment and Sentinel implementation through
all channels already required by Sections 1 and 12–16:

- **GitHub:** commit the updated `patch.md`, Constitution, Laws, GUILD-039 charter,
  SPECIALIST-033 charter, SO-026 workflow, implementation, schemas, migrations, tests,
  and docs through a protected pull request with non-Sentinel reviewers;
- **Google Drive:** upload the reviewed `patch.md`, constitutional amendment, test
  evidence, sample redacted reports, closure record, and exact signed release archive;
- **Wiki:** publish the Article VIII no-guild-trust rule, Sentinel operator guide,
  assurance-failure behavior, safe testing boundary, and remediation workflow, and add
  all pages to the wiki index; and
- **Next full build:** include every Sentinel and constitutional artifact in the source
  distribution and package data, list each in `RELEASE-MANIFEST.json` and `SHA256SUMS`,
  reference the change in `CHANGELOG.md` and release notes, and verify the installed
  offline wheel exposes the same behavior.

Hermes must record the Drive, GitHub, wiki, build, test, and artifact links in Section
16. Until Sections 18.1–18.7 pass independent review, the feature must remain a
**development candidate** and may not be described as a production security control.

### 18.9 Acceptance statement for this additive feature

Sentinel is accepted only when its own work demonstrably receives the same ordinary
adversarial treatment as work from every other new guild. “Security guild,”
“independent organization,” or “penetration-testing function” is never a trust bypass.
The feature is additive defense-in-depth and leaves every original audit finding open
until that finding's separate closure criteria and re-audit gate pass.
